...
class JDetloss(torch.nn.Module):
    """
    Jacobian determinant loss to penalize irregular transformations in the deformation field.
    """
    def __init__(self, eps=1e-3):
        super(JDetloss, self).__init__()
        self.eps = eps

    def forward(self, displacement):
        """
        displacement: Tensor of shape (B, 2, H, W) for 2D displacement field.
        """
        # Compute gradients along each spatial dimension
        dx = displacement[:, :, 1:, :-1] - displacement[:, :, :-1, :-1]
        dy = displacement[:, :, :-1, 1:] - displacement[:, :, :-1, :-1]

        # Compute Jacobian determinant
        Jdet = dx[:, 0, :, :] * dy[:, 1, :, :] - dx[:, 1, :, :] * dy[:, 0, :, :]
        
        # Penalize regions with negative or zero determinants (non-invertible transformations)
        # Optional: Clamp Jdet to avoid extreme values
        Jdet_clamped = torch.clamp(Jdet, min=self.eps)
        
        # Loss is the mean of the negative determinants
        loss = torch.mean(F.relu(-Jdet_clamped))
